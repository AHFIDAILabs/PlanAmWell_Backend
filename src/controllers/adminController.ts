// controllers/adminController.ts - UPDATED
...
export const getAllUsersAdmin = asyncHandler(async (_req: Request, res: Response) => {
  const users = await User.find().select("-password").populate("userImage");
...
export const getCombinedGrowth = asyncHandler(async (req: Request, res: Response) => {
  ...
  const lastMonthPending = monthlyGrowth[monthlyGrowth.length - 2]?.pendingDoctors || 0;
  const currentMonthPending = monthlyGrowth[monthlyGrowth.length - 1]?.pendingDoctors || 0;
