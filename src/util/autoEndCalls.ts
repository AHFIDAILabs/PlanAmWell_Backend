...
export const resetZombieRingingCalls = async () => {
  try {
    ...
    if (result.modifiedCount > 0) {
      console.log(`🔄 [ZombieReset] Reset ${result.modifiedCount} stuck ringing calls to idle`);
    }
...
export const autoEndExpiredCalls = async () => {
  try {
    ...
    for (const appointment of expiredCalls) {
      ...
      const callDuration = Math.floor((now.getTime() - new Date(callStart).getTime()) / 1000);
      ...
      try {
        await NotificationService.notifyCallAutoEnded(doctorId,  'Doctor', apptId, callDuration);
        await NotificationService.notifyCallAutoEnded(patientId, 'User',   apptId, callDuration);
      } catch (notifError) {
        console.error(`[AutoEnd] Failed to notify for ${apptId}:`, notifError);
      }

      endedCount++;
      console.log(`⏰ [AutoEnd] Force-ended call for appointment ${apptId} after ${HARD_CAP_HOURS}h — appointment still open`);
    }
  ...
  try {
    ...
    for (const appointment of activeCalls) {
      ...
      if (minutesUntilEnd > 0 && minutesUntilEnd <= 10) {
        ...
        try {
          await NotificationService.notifyCallExpiryWarning(doctorId,  'Doctor', apptId, minutesUntilEnd);
          await NotificationService.notifyCallExpiryWarning(patientId, 'User',   apptId, minutesUntilEnd);
          await NotificationService.markNotificationSent(apptId, 'expiryWarning');
          warningCount++;
        } catch (notifError) {
